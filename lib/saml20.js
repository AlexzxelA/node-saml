...
var saml20nameID20 = fs.readFileSync(path.join(__dirname, 'saml20nameID20.template')).toString();
...
exports.create = function(options, callback) {
	...
	var doc;
	try {
		doc = (options.nameIDPersistent)?new Parser().parseFromString(saml20.toString()):
			new Parser().parseFromString(saml20nameID20.toString());
	} catch(err){
	...
	if (options.attributes) {
		...
		Object.keys(options.attributes).forEach(function(prop) {
			...
			console.log('Prop:'+prop +' attr:'+options.attributes[prop]);
	...
	if (options.nameIdentifier || options.persistentId) {
		nameID.textContent = options.persistentId||options.nameIdentifier;
	}
	...
	if (options.spNameQualifier) {
		nameID.setAttribute('SPNameQualifier', options.spNameQualifier);
	}
